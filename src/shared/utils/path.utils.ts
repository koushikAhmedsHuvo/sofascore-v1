...
export function classifyEndpoint(path: string): EndpointType {
  ...
  if (
    p.match(/event\/\d+\/(incidents|statistics|graph\/win-probability|graph|point-by-point|team-streaks|points\/overview)$/)
  ) {
    return EndpointType.LIVE;
  }
  ...
  if (
    ...
    p.match(/unique-tournament\/\d+\/(seasons?|standings|cuptrees|top-players|top-ratings)/) ||
    // Simple tournament detail
    p.match(/^unique-tournament\/\d+$/) ||
    // Season-level sub-paths
    p.match(/unique-tournament\/\d+\/season\/\d+\/(team-events|cuptrees|top-players|top-players-per-game|top-ratings)/) ||
    p.match(/tournament\/\d+\/season\/\d+\/standings/) ||
    // Team profile sub-paths (all fairly static)
    p.match(/team\/\d+\/(rankings|team-statistics|recent-unique-tournaments|performance|featured-event|featured-players|players|transfers|media)/) ||
    p.match(/team\/\d+\/unique-tournament\/\d+\/season\/\d+\/(statistics|top-players)/) ||
    p.match(/^team\/\d+$/) ||
    // Player static data
    p.match(/^player\/\d+\/(performance|attribute-overviews|statistics-seasons|unique-tournaments|national-team-statistics|transfer-history)$/) ||
  ...
  if (
    p.match(/event\/\d+/) ||
    p.match(/team\/\d+\/events\/(next|last)/)
  ) {
    return EndpointType.RECENT;
  }
